    ...
    switch (node.type) {
      ...
      case DELETE:
        switch (node.children[0].type) {
          ...
          case INDEX:
            {
                var def_9302 = resolvePath(node, env);
                if (def_9302)
      ...
      case COMMA:
      case LIST:
        for (var i_3204 in node.children)
            resolveExpression(node.children[i_3204], env);
        break;
      ...
      case ARRAY_INIT:
        for (var i_4204 in node.children) {
            var elt_4305 = node.children[i_4204];
            if (elt_4305)
                resolveExpression(elt_4305, env);
        }
        break;

      case OBJECT_INIT:
        for (var i_4950 in node.children)
            resolveExpression(node.children[i_4950].children[1], env);
        break;
...
function resolveStatement(node, env) {
    switch (node.type) {
      ...
      case SWITCH:
        ...
        for (var i_4920 = 0; i_4920 < node.cases.length; i_4920++) {
            var caseNode_7752 = node.cases[i_4920];
            if (caseNode_7752.caseLabel)
                resolveExpression(caseNode_7752.caseLabel, env);
            resolveStatement(caseNode_7752.statements, env);
        }
        break;
      ...
      case TRY:
        ...
        for (var i_4920 = 0; i_4920 < node.catchClauses.length; i_4920++)
            resolveCatch(node.catchClauses[i_4920], env);
